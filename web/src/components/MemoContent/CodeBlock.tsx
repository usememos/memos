...
const CodeBlock: React.FC<Props> = ({ language, content }: Props) => {
  ...
  if (formatedLanguage === SpecialLanguage.HTML) {
    ...
    return <div className="w-full overflow-auto !my-2" dangerouslySetInnerHTML={{ __html: purify.sanitize(content, { ALLOWED_TAGS: ["iframe"], ADD_ATTR: ['allow', 'allowfullscreen', 'frameborder', 'scrolling'] }) }} />;
