...
export const remarkTag = () => {
  return (tree: Root) => {
    visit(tree, "text", (node: Text, index, parent) => {
      ...
      // Replace text node with multiple nodes (text + html for tags)
      const newNodes = segments.map((segment) => {
        if (segment.type === "tag") {
          // Create HTML node for tag
          return {
            type: "html" as const,
            value: `<span class="tag" data-tag="${escapeHtml(segment.value)}">#${escapeHtml(segment.value)}</span>`,
          };
        } else {
          // Keep as text node
          return {
