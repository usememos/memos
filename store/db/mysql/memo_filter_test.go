...
func TestConvertExprToSQL(t *testing.T) {
	tests := []struct {
		...
		{
			filter: `!has_incomplete_tasks`,
			want:   "NOT (JSON_EXTRACT(`memo`.`payload`, '$.property.hasIncompleteTasks') = CAST('true' AS JSON))",
			args:   []any{},
		},
		{
			filter: `has_incomplete_tasks && pinned`,
			want:   "(JSON_EXTRACT(`memo`.`payload`, '$.property.hasIncompleteTasks') = CAST('true' AS JSON) AND `memo`.`pinned` IS TRUE)",
			args:   []any{},
		},
		{
			filter: `has_incomplete_tasks && content.contains("todo")`,
			want:   "(JSON_EXTRACT(`memo`.`payload`, '$.property.hasIncompleteTasks') = CAST('true' AS JSON) AND `memo`.`content` LIKE ?)",
			args:   []any{"%todo%"},
		},
    {
