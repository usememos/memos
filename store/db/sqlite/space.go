...
func (d *DB) CreateSpace(ctx context.Context, create *store.Space, creatorID int32) (*store.Space, error) {
	...
	if _, err := tx.ExecContext(ctx, "INSERT INTO space_member (space_id, user_id, role) VALUES (?, ?, ?)", space.ID, creatorID, store.SpaceMemberRoleAdmin); err != nil {
...
func (d *DB) CreateSpaceMember(ctx context.Context, create *store.SpaceMember, actorUserID int32) (*store.SpaceMember, error) {
	...
	query := `INSERT INTO space_member (space_id, user_id, role) VALUES (?, ?, ?)
		RETURNING space_id, user_id, role`
	member := &store.SpaceMember{}
	if err := tx.QueryRowContext(ctx, query, create.SpaceID, create.UserID, create.Role).Scan(&member.SpaceID, &member.UserID, &member.Role); err != nil {
		if isSQLiteUniqueViolation(err) {
...
func (d *DB) UpdateSpaceMember(ctx context.Context, update *store.UpdateSpaceMember, actorUserID int32) (*store.SpaceMember, error) {
	...
	if err := tx.QueryRowContext(ctx, "SELECT role FROM space_member WHERE space_id = ? AND user_id = ?", update.SpaceID, update.UserID).Scan(&currentRole); err != nil {
	...
	query := `UPDATE space_member SET ` + strings.Join(sets, ", ") + ` WHERE space_id = ? AND user_id = ?
		RETURNING space_id, user_id, role`
	member := &store.SpaceMember{}
...
func (d *DB) DeleteSpaceMember(ctx context.Context, delete *store.DeleteSpaceMember, actorUserID int32) error {
	...
	if actorUserID != delete.UserID {
		...
		if err := tx.QueryRowContext(ctx, "SELECT sm.role FROM space_member sm JOIN user u ON u.id = sm.user_id WHERE sm.space_id = ? AND sm.user_id = ? AND u.row_status = 'NORMAL'", delete.SpaceID, actorUserID).Scan(&actorRole); errors.Is(err, sql.ErrNoRows) {
	...
	if err := tx.QueryRowContext(ctx, "SELECT sm.role, u.row_status FROM space_member sm LEFT JOIN user u ON u.id = sm.user_id WHERE sm.space_id = ? AND sm.user_id = ?", delete.SpaceID, delete.UserID).Scan(&role, &targetStatus); err != nil {
		if errors.Is(err, sql.ErrNoRows) {
...
func authorizeSQLiteSpaceAdmin(ctx context.Context, tx dbExecutor, spaceID, actorUserID int32) error {
	...
	var role store.SpaceMemberRole
	if err := tx.QueryRowContext(ctx, "SELECT sm.role FROM space_member sm JOIN user u ON u.id = sm.user_id WHERE sm.space_id = ? AND sm.user_id = ? AND u.row_status = 'NORMAL'", spaceID, actorUserID).Scan(&role); errors.Is(err, sql.ErrNoRows) {
		return store.ErrSpacePermissionDenied
	} else if err != nil {
		return err
