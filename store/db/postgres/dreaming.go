...
func (d *DB) ListDreamingRuns(ctx context.Context, find *store.FindDreamingRun) ([]*store.DreamingRun, error) {
	...
	if find.RunType != nil {
		where, args = append(where, "run_type = "+placeholder(len(args)+1)), append(args, *find.RunType)
	}
	if find.Status != nil {
		where, args = append(where, "status = "+placeholder(len(args)+1)), append(args, *find.Status)
	}
	if find.LockHolder != nil {
		where, args = append(where, "lock_holder = "+placeholder(len(args)+1)), append(args, *find.LockHolder)
	}
...
func (d *DB) UpdateDreamingRun(ctx context.Context, update *store.UpdateDreamingRun) error {
	...
	if update.FinishedAt != nil {
		set, args = append(set, "finished_at = "+placeholder(len(args)+1)), append(args, *update.FinishedAt)
	}
	if update.LockHolder != nil {
		set, args = append(set, "lock_holder = "+placeholder(len(args)+1)), append(args, *update.LockHolder)
	}
	if update.ErrorMessage != nil {
		set, args = append(set, "error_message = "+placeholder(len(args)+1)), append(args, *update.ErrorMessage)
	}
	if update.StatsJSON != nil {
		set, args = append(set, "stats_json = "+placeholder(len(args)+1)), append(args, *update.StatsJSON)
	}
	if update.Phase != nil {
		set, args = append(set, "phase = "+placeholder(len(args)+1)), append(args, *update.Phase)
	}
	if len(set) == 0 {
		return nil
	}
	...
	query := fmt.Sprintf("UPDATE dreaming_runs SET %s WHERE id = %s", strings.Join(set, ", "), placeholder(len(args)))
...
func (d *DB) DeleteDreamingRun(ctx context.Context, delete *store.DeleteDreamingRun) error {
	_, err := d.db.ExecContext(ctx, "DELETE FROM dreaming_runs WHERE id = "+placeholder(1), delete.ID)
...
func (d *DB) ListDreamingReplayQueueItems(ctx context.Context, find *store.FindDreamingReplayQueueItem) ([]*store.DreamingReplayQueueItem, error) {
	...
	if find.ChunkID != nil {
		where, args = append(where, "chunk_id = "+placeholder(len(args)+1)), append(args, *find.ChunkID)
	}
	if find.SessionKey != nil {
		where, args = append(where, "session_key = "+placeholder(len(args)+1)), append(args, *find.SessionKey)
	}
	if find.Status != nil {
		where, args = append(where, "status = "+placeholder(len(args)+1)), append(args, *find.Status)
	}
	if find.SpindleTag != nil {
		where, args = append(where, "spindle_tag = "+placeholder(len(args)+1)), append(args, *find.SpindleTag)
	}
...
func (d *DB) UpdateDreamingReplayQueueItem(ctx context.Context, update *store.UpdateDreamingReplayQueueItem) error {
	...
	if update.SpindleTag != nil {
		set, args = append(set, "spindle_tag = "+placeholder(len(args)+1)), append(args, *update.SpindleTag)
	}
	if update.DreamingCandidateScore != nil {
		set, args = append(set, "dreaming_candidate_score = "+placeholder(len(args)+1)), append(args, *update.DreamingCandidateScore)
	}
	if update.CandidateKindHint != nil {
		set, args = append(set, "candidate_kind_hint = "+placeholder(len(args)+1)), append(args, *update.CandidateKindHint)
	}
	if update.LastReplayedAt != nil {
		set, args = append(set, "last_replayed_at = "+placeholder(len(args)+1)), append(args, *update.LastReplayedAt)
	}
	if update.RehearsalCount != nil {
		set, args = append(set, "rehearsal_count = "+placeholder(len(args)+1)), append(args, *update.RehearsalCount)
	}
	if len(set) == 0 {
		return nil
	}
	...
	args = append(args, update.ID)
	query := fmt.Sprintf("UPDATE dreaming_replay_queue SET %s WHERE id = %s", strings.Join(set, ", "), placeholder(len(args)))
	_, err := d.db.ExecContext(ctx, query, args...)
...
func (d *DB) DeleteDreamingReplayQueueItem(ctx context.Context, delete *store.DeleteDreamingReplayQueueItem) error {
	_, err := d.db.ExecContext(ctx, "DELETE FROM dreaming_replay_queue WHERE id = "+placeholder(1), delete.ID)
...
func (d *DB) ListDreamingInsights(ctx context.Context, find *store.FindDreamingInsight) ([]*store.DreamingInsight, error) {
	...
	if find.MemoryClass != nil {
		where, args = append(where, "memory_class = "+placeholder(len(args)+1)), append(args, *find.MemoryClass)
	}
	if find.Status != nil {
		where, args = append(where, "status = "+placeholder(len(args)+1)), append(args, *find.Status)
	}
	if find.Kind != nil {
		where, args = append(where, "kind = "+placeholder(len(args)+1)), append(args, *find.Kind)
	}
	if find.Phase != nil {
		where, args = append(where, "phase = "+placeholder(len(args)+1)), append(args, *find.Phase)
	}
	if find.RetrievalPriorityGreaterThan != nil {
		where, args = append(where, "retrieval_priority > "+placeholder(len(args)+1)), append(args, *find.RetrievalPriorityGreaterThan)
	}
...
func (d *DB) UpdateDreamingInsight(ctx context.Context, update *store.UpdateDreamingInsight) error {
	...
	if update.SalienceScore != nil {
		set, args = append(set, "salience_score = "+placeholder(len(args)+1)), append(args, *update.SalienceScore)
	}
	if update.RetrievalPriority != nil {
		set, args = append(set, "retrieval_priority = "+placeholder(len(args)+1)), append(args, *update.RetrievalPriority)
	}
	if update.DecayFactor != nil {
		set, args = append(set, "decay_factor = "+placeholder(len(args)+1)), append(args, *update.DecayFactor)
	}
	if update.SupportCount != nil {
		set, args = append(set, "support_count = "+placeholder(len(args)+1)), append(args, *update.SupportCount)
	}
	if update.LastReinforcedAt != nil {
		set, args = append(set, "last_reinforced_at = "+placeholder(len(args)+1)), append(args, *update.LastReinforcedAt)
	}
	if update.MergedInto != nil {
		set, args = append(set, "merged_into = "+placeholder(len(args)+1)), append(args, *update.MergedInto)
	}
	if len(set) == 0 {
		return nil
	}

	set = append(set, "updated_at = "+placeholder(len(args)+1))
	args = append(args, store.GetCurrentTimestamp())
	args = append(args, update.ID)
	query := fmt.Sprintf("UPDATE dreaming_insights SET %s WHERE id = %s", strings.Join(set, ", "), placeholder(len(args)))
	_, err := d.db.ExecContext(ctx, query, args...)
...
func (d *DB) DeleteDreamingInsight(ctx context.Context, delete *store.DeleteDreamingInsight) error {
	_, err := d.db.ExecContext(ctx, "DELETE FROM dreaming_insights WHERE id = "+placeholder(1), delete.ID)
...
func (d *DB) ListDreamingInsightEvidences(ctx context.Context, find *store.FindDreamingInsightEvidence) ([]*store.DreamingInsightEvidence, error) {
	...
	if find.InsightID != nil {
		where, args = append(where, "insight_id = "+placeholder(len(args)+1)), append(args, *find.InsightID)
	}
	if find.ChunkID != nil {
		where, args = append(where, "chunk_id = "+placeholder(len(args)+1)), append(args, *find.ChunkID)
	}
...
func (d *DB) UpsertDreamingInsightEmbedding(ctx context.Context, upsert *store.UpsertDreamingInsightEmbedding) error {
	_, err := d.db.ExecContext(ctx,
		"INSERT INTO dreaming_insight_embeddings (insight_id, embedding, created_at) VALUES ("+placeholders(3)+") ON CONFLICT (insight_id) DO UPDATE SET embedding = EXCLUDED.embedding, created_at = EXCLUDED.created_at",
