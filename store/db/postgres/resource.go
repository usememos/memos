...
func (d *DB) CreateResource(ctx context.Context, create *store.Resource) (*store.Resource, error) {
	qb := squirrel.Insert("resource").
		Columns("filename", "blob", "external_link", "type", "size", "creator_id", "internal_path").
		Values(create.Filename, create.Blob, create.ExternalLink, create.Type, create.Size, create.CreatorID, create.InternalPath)
	...
	if create.CreatedTs != 0 {
		qb = qb.Columns("created_ts").Values(time.Unix(0, create.CreatedTs))
	}

	if create.UpdatedTs != 0 {
		qb = qb.Columns("updated_ts").Values(time.Unix(0, create.UpdatedTs))
	}

	if create.MemoID != nil {
		qb = qb.Columns("memo_id").Values(*create.MemoID)
	}
