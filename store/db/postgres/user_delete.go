...
func deleteHabitsByCreatorTx(ctx context.Context, tx *sql.Tx, userID int32) error {
	if _, err := tx.ExecContext(ctx, "DELETE FROM habit_log WHERE habit_id IN (SELECT id FROM habit WHERE creator_id = $1)", userID); err != nil {
		return err
	}
	_, err := tx.ExecContext(ctx, "DELETE FROM habit WHERE creator_id = $1", userID)
	return err
}
