-- Schema for Cloudflare D1. D1 runs SQLite, enforces foreign keys by default,
-- and has no interactive transactions; see store/db/d1/doc comments.

-- d1_guard backs conditional writes inside atomic batches: inserting 0
-- violates the CHECK and aborts the whole batch. It never holds rows.
CREATE TABLE d1_guard (
  ok INTEGER NOT NULL CONSTRAINT d1_guard_ok CHECK (ok = 1)
);
